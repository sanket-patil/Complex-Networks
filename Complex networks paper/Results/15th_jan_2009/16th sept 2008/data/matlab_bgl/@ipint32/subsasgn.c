/*
 * =============================================================
 * ipint32/subsasgn.c Change the vector/matrix/array in place.
 *
 * David Gleich
 * Stanford University
 * 1 June 2006
...
void assign(mxArray* lhs, mxArray* subs, mxArray* rhs)
{
    ...
    pr = (int*)mxGetData(rhs);
    pl = (int*)mxGetData(lhs);
                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    if (!mxIsClass(prhs[0],"ipint32"))
        mexErrMsgTxt("Input 1 must be an ipint32.");
    ...
    if (!mxIsInt32(prhs[2]))
        mexErrMsgTxt("Only int32 values supported right now!");
    
    a = mxGetField(prhs[0],0,"a");
    if (!a)
        mexErrMsgTxt("Input 1 is not a valid ipint32 class.");
    
    if (!mxIsInt32(a))
        mexErrMsgTxt("Input 1 is not a valid ipint32 class (a is not a int32).");
