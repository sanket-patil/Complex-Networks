...
def produceOffsprings(params, matingpool, offsprings):
    CLONE = 0.1
    EQED = 0.20
    TWOPT = 0.40
    CNS = 0.50    
...
##        w = True
    children = []
##    gr1 = graph.graph.graph(d, w)
##    gr2 = graph.graph.graph(d, w)
    gr1 = graph.graph.graph()
    gr2 = graph.graph.graph()
    for m1 in matingpool:
        for m2 in matingpool:
            ...
            if cotype <= CLONE:
                ...
                equalNodesCO(m1, m2, children, gr1, gr2, n)
            elif cotype <= CPED:
                copyEdgesCO(m1, m2, children, gr1, gr2, n)
            elif cotype <= BFED:
                bestFitEdgesCO(m1, m2, children, gr1, gr2, n)
            else:
                bestFitNodesCO(m1, m2, children, gr1, gr2, n)
            if children:
...
def equalNodesCO(m1, m2, children, gr1, gr2, n):
    if m1 and m2:
        gr1.create(m1, n)
        gr2.create(m2, n)
        nodes1 = gr1.getNodes()
...
def copyEdgesCO(m1, m2, children, gr1, gr2, n):
    if m1 and m2:
        gr1.create(m1, n)
        gr2.create(m2, n)
        nodes1 = gr1.getNodes()
...
def transferEdgesCO(m1, m2, children, gr1, gr2, n):
    if m1 and m2:
        gr1.create(m1, n)
        gr2.create(m2, n)
        nodes1 = gr1.getNodes()
